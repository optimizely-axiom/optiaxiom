...
If the `execute` property is also present then the default link clicking behavior will be prevented (unless users use a modifier like <Kbd keys="command">Enter</Kbd> to open in a new tab).
