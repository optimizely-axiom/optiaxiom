...
The calendar popover is localized to the browser locale (via `navigator.language`) by default, which controls the captions, weekday headers, week start, and AM/PM labels. Override the locale for the whole app by passing the `locale` prop to [AxiomProvider](../axiom-provider/#locale).
...
<Cards items={["Field", "Input", "DateRangePicker"]} />
