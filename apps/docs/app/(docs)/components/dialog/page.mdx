...
import { Alert, Badge, Kbd } from "@optiaxiom/react";
...
<Cards items={["AlertDialog", "Popover"]} />
...
<KeyboardTable
  interactions={[
    {
      keys: ["Space", "Enter"],
      description: "When focus is on `DialogTrigger`, opens the dialog.",
    },
    {
      keys: "Esc",
      description: "Closes the dialog and moves focus to `DialogTrigger`.",
    },
  ]}
/>
