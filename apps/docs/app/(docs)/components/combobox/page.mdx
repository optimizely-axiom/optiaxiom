...
The `ComboboxContent` component further includes the input, a listbox, and an empty section.
...
By default content popover will render an input and the provided items. But we can customize how items are rendered using the `ComboboxScrollArea` component.

`ComboboxScrollArea` accepts a react node as well as a render prop where the first parameter is the item that needs to be rendered.
...
<Demo component="combobox/overview/empty-usage" meta="{35}" />
...
We can use virtualization to improve performance when rendering a large number of items inside the combobox. Inside `ComboboxScrollArea` set `children` to a render prop to enable virtualization.

<Demo component="combobox/virtualized-usage" meta="{49-51}" />
