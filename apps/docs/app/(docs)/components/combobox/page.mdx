...
The content popover will be empty until we render the same items we provided to the `items` prop. Items need to be rendered inside a `ComboboxScrollArea` component.

#### Multi-select

We can render multi-select items using `ComboboxCheckboxItem`.

<Demo component="combobox/overview/content-checkbox-usage" meta="{16-22}" />

#### Single-select

We can render single-select items using `ComboboxRadioItem`.

<Demo component="combobox/overview/content-radio-usage" meta="{18,20}" />
...
<Demo component="combobox/overview/input-usage" meta="{14,19-25,29}" />
...
Pressing enter or clicking items will not do anything in previous demos. For that we need to add the `value` and `onItemSelect` props to the main `Combobox` component.

#### Multi-select

<Demo component="combobox/overview/value-checkbox-usage" meta="{15,27-34}" />

#### Single-select

<Demo component="combobox/overview/value-radio-usage" meta="{15,27-30}" />

### Empty

Finally we can render an empty state in the content using the `ComboboxEmpty` component.

<Demo component="combobox/overview/empty-usage" meta="{37}" />
