...
export function injectGlobalStyle(text: string) {
  ...
  style.appendChild(
    document.createTextNode(
      text
        .replaceAll(
          /font-family:\s?(DI Grotesk B|Nudge Semicondensed|Roboto Condensed Variable|Roboto Mono Variable|Roboto Variable+);/g,
...
export function injectLocalStyle(text: string) {
  styles.push(
    text.replaceAll(
      /DI Grotesk B|Nudge Semicondensed|Roboto Condensed Variable|Roboto Mono Variable|Roboto Variable/g,
      (m) => `${uuid} ${m}`,
    ),
  );
