...
- 1a1d7ff: Respect the `disabled` prop on `Input` and `Textarea` for the AI suggestion
...
- Updated dependencies [20438fd]
  - @optiaxiom/globals@1.1.13

## 1.9.39

### Patch Changes

- 07e208c: Clarify in `Menu`'s documentation that it is not a form control (no `value`/`onChange` or `Field` integration) and that there is currently no built-in filterable single-select form input. This helps when choosing between `Menu` (filterable) and `Select` (form control) for a single-select field.
- 9a94689: Clarify the descriptions of the `flex`, `gridTemplateColumns`, `gridColumn`, `gridAutoRows`, and `transition` style props, where the accepted value does not obviously map to its CSS intent. For example, `flex="none"` now documents that it neither grows nor shrinks (the replacement for `flex-shrink: 0`), and `gridTemplateColumns` documents that the value is the number of equal columns. These flow to IDE hovers, generated docs, and the MCP.

## 1.9.38

### Patch Changes

- 9c305bc: default input to always be visible for PillMenu
...
- ae51b5d: Add disabled pill support to PillMenu
- b386822: fix extracting style prop in extractBoxProps
...
- 6848710: add HoverCard component
