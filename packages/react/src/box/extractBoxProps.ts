...
export function extractBoxProps<S extends Record<string, unknown>>(props: S) {
  const boxProps = {} as Pick<
    S,
    ("className" | "style" | keyof Sprinkles) & keyof S
  >;
  const restProps = {} as Omit<S, "className" | "style" | keyof Sprinkles>;

  for (const [name, value] of Object.entries(props)) {
    if (
      name === "className" ||
      name === "style" ||
      sprinkles.properties.has(name as never)
    ) {
